...
class TestFantasticFourBug:
    ...
    def test_number_word_conversion_in_titles(self):
        ...
        # Debug output
        print(
            f"Match result: is_matched={result.is_matched}, method={result.match_method}, confidence={result.confidence}"
        )
        if result.is_matched:
            print(
                f"Matched movie: id={result.radarr_movie.id}, title={result.radarr_movie.title}"
            )

        # This should match the first movie (The Fantastic 4: First Steps)
        assert (
            result.is_matched
        ), f"Failed to match 'The Fantastic Four: First Steps'. Method: {result.match_method}, Confidence: {result.confidence}"
        assert (
            result.radarr_movie.id == 617126
        ), f"Wrong movie matched: got id={result.radarr_movie.id}, expected 617126"
    ...
    def test_various_number_word_conversions(self):
        ...
        for box_office_title, expected_radarr_title in test_cases:
            ...
            assert (
                result.is_matched
            ), f"Failed to match '{box_office_title}' to '{expected_radarr_title}'"
    ...
    def test_partial_title_with_number_word_conversion(self):
        ...
        result = matcher.match_single(
            "The Fantastic Four: Rise of the Silver Surfer", radarr_movies
        )
