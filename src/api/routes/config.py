    ...
    try:
        ...
        try:
            ...
            if scheduler and hasattr(scheduler, '_running') and scheduler._running:
                ...
                if schedule_changed or enabled_changed:
                    if new_enabled:
                        ...
                        if scheduler.reload_schedule(new_cron):
                            logger.info(f"✅ Scheduler reloaded: {old_cron} → {new_cron}")
                        else:
                            logger.error(f"Failed to reload scheduler with new cron: {new_cron}")
                    else:
                ...
                logger.warning("⚠️ Scheduler should be enabled but instance not found - restart required")
        except Exception as e:
