"""Box Office Mojo provider supporting all international markets."""
...
class BoxOfficeMojoProvider(BoxOfficeProvider):
    """
    Box office data provider using Box Office Mojo.

    Supports all international markets via the ?area= parameter.
    The US market uses no area parameter (default BOM behavior).
    All markets share the same HTML table structure and use USD.
    """
    ...
    def get_weekend_dates(
        ...
        # If today is Fri/Sat/Sun, data isn't available yet — use previous weekend
        if weekday in (4, 5, 6):
            days_since_friday += 7
    ...
    def fetch_weekend_box_office(
        ...
        """Fetch box office data from Box Office Mojo."""
    ...
    def _extract_imdb_id(self, release_url: str) -> Optional[str]:
        """Fetch a BOM release page and extract the IMDb ID."""
        try:
