    ...
    def _try_fuzzy_match(
        ...
        for movie in radarr_movies:
            ...
            normalized_score = self.calculate_similarity(
                normalized_title,
                self.normalize_title(movie.title)
            )
            base_score = self.calculate_similarity(
                self.get_base_title(title),
                self.get_base_title(movie.title)
            )
        ...
        if year_match:
            ...
            for movie in radarr_movies:
                if (movie.year == year and 
                    self.calculate_similarity(title_no_year, movie.title) > 0.8):
                    return movie
